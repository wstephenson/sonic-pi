...
if $task == :extract then
  ...
  lang =
    Dir[File.expand_path("#{doc_lang_dir}/sonic-pi-tutorial-*.po", __dir__)].
    map { |p| File.basename(p).gsub(/sonic-pi-tutorial-(.*?).po/, '\1') }.sort
end

lang.each do |l|

  po_filename  = File.expand_path("#{doc_lang_dir}/sonic-pi-tutorial-#{l}.po", __dir__)
  pot_filename = File.expand_path("#{doc_lang_dir}/sonic-pi-tutorial.pot", __dir__)
  ...
  if $task == :update then
    ...
    if $task == :translate then
      ...
      parser.parse_file(File.expand_path("#{doc_lang_dir}/sonic-pi-tutorial-#{l}.po", __dir__), $po)
    end

    Dir[File.expand_path("#{doc_dir}/tutorial/*.md", __dir__)].sort.each do |path|
    ...
    case $task
    when :translate
      doc_generated_lang_tutorial_dir = "#{doc_dir}/generated/#{l}/tutorial"
      FileUtils::rm_rf File.expand_path(doc_generated_lang_tutorial_dir, __dir__)
      FileUtils::mkdir_p File.expand_path(doc_generated_lang_tutorial_dir, __dir__)
      $translated.each do |filename, newcontent|
        File.open(File.expand_path(doc_generated_lang_tutorial_dir +"/#{filename}", __dir__), 'w') do |f|
          f << newcontent
        end
      end
    ...
    when :extract
      pot_file_path = "#{doc_lang_dir}/sonic-pi-tutorial.pot"
      File.open(File.expand_path(pot_file_path, __dir__), 'w') do |f|
        $stderr.puts "Writing #{pot_file_path}"
        f << <<-HEADER
